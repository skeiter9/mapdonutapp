...
export default function runI18next(
  ...
  const _i18n = (_options: InitOptions) =>
    i18next
      .use(Locize)
  ...
  const toPromise = (attr: 'getOptions' | 'getLanguages'): Promise<any> =>
    new Promise((resolve, reject) => {
      locizeService[attr]((err, res) => (!!err ? reject(err) : resolve(res)));
    });

  return Promise.all([toPromise('getOptions')])
    .then(([optsServer]) => {
      return Promise.all([
