...
import { Navigation, LayoutComponent, OptionsModalPresentationStyle, OptionsModalTransitionStyle, LayoutBottomTabs } from "react-native-navigation";
import { SCREEN_LOGIN, SCREEN_PROFILE, SCREEN_EXPLORE, SCREEN_SETTINGS, SCREEN_SPLASH } from "../screenNames";
...
export const showRootOverlay = () => Navigation.showOverlay({
    component: {
        id: 'loader',
        name: SCREEN_SPLASH,
        passProps: {
...
export const LoginRnnComponent = (passProps: LoginProps): LayoutComponent => ({
    ...
    options: {
        ...
        customTransition: {
            animations: [
                {
                    type: 'sharedElement', fromId: 'LOGO_SLASH', toId: 'LOGO_LOGIN',
                    startDelay: 0, springVelocity: 10, duration: 15
                }
            ],
            duration: .5
        }
...
export const goToLogin = (bgLogin: string) => of(null).pipe(
    switchMap(() => {
        const root = () => Navigation.setRoot({
            root: {
...
export const goToHome = () => of(null).pipe(
    switchMap(() => {
    ...
    switchMap(({ icons }) => {
        const exec = () => Promise.resolve(null)
            .then(() => Navigation.setStackRoot('MAIN_STACK', [{
