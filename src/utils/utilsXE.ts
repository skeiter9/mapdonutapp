...
import {
  currenciesSupportedByXE,
  Mapdonut_Currency_Code,
} from '../mapdonut_libs/currencies/currencies_types';
...
const fetchApi = (url: string, cred: XE_cred): Promise<XE_response> =>
  fetch(url, {
    method: 'GET',
    headers: new Headers({
...
export function XEgetExchange(
  cred: XE_cred,
  from: Mapdonut_Currency_Code,
  amount: number = 100,
) {
  return fetchApi(
...
export function XEfromTo(
  cred: XE_cred,
  from: Mapdonut_Currency_Code,
  to: Mapdonut_Currency_Code,
  amount: number = 100,
) {
  if (
    !contains(from, currenciesSupportedByXE) ||
    !contains(from, currenciesSupportedByXE)
  ) {
