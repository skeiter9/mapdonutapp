...
export default function Tutorial(props: Props) {
    ...
    const _renderItem = ({ item }: any) => (<Surface
        style={{
            height: HEIGHT,
            borderRadius: 16,
            padding: 16,
    ...
    >
        <Image source={{ uri: item.img }} style={{ width: '100%', height: item.imgIsRectangle ? 200 : 290 }}  />
        <View>
    ...
    return (
        ...
        >
            ...
            <Button color={themeVariables.colors.text} style={{ alignSelf: 'center', marginTop: 16}}
                onPress={() => closeModal(componentId)} mode="text" compact
            >
                {words.SKIP_TUTORIAL}
            </Button>
        </Animated.View>
