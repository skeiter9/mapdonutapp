...
export default function Settings(props: Props) {
    ...
    return <View>
        <Animated.ScrollView bounces={false} style={{
        ...
        >
            <SimpleSetting onPress={() => setShowLangs(true)} title={words.LANGUAGE} value={words[`LANG_${lang.toUpperCase()}`]} />
            <Hr mb={16} />

            <SimpleSetting onPress={() => setShowThemes(true)} title={words.THEME} value={theme} />
            <Hr mb={16} />

            <SimpleSetting onPress={() => dispatch({type: 'OPEN_PASSPORTS', payload: {}})} title={words.PASSPORT} value={''} placeholder={words.SET_PASSPORT}
                valueComponent={passports.length === 0 ? null : <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    {passports.map((p, i) => <FlagPassport countryCode={p.countryCode} key={i} />)}
                </View>}
            />
            <Hr mb={16} />

            <SimpleSetting onPress={() => dispatch({ type: 'OPEN_CURRENCIES', payload: {} })}  title={words.CURRENCIES} value={currencies} />
            <Hr mb={16} />

            <SimpleSetting onPress={() => {}} title={words.UNITS} value={''} />
            <UserSettingsUnits />

            <Button color={themeVariables.colors.disabled}
                mode='contained'
        ...
        <BsSimple snaps={[180, 0]} show={showThemes} setShow={setShowThemes}>
            <ThemesList width={widthTheme} height={heightTheme} onPress={(themeName) => {
