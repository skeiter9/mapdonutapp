...
export default function(props: LoginProps) {
  ...
  const {
    ...
    words,
    lang,
  } = viewSelector(useSelector(_selectorStore), props);
  const [bgVideo, setBgVideo] = useState(bgLogin || 'video_6');
  return (
    <VideoBackground uri={bgVideo} colors={bgColors}>
      <>
        <StatusBar
        ...
        <LinearGradient
          ...
          <View
            ...
            <Button
              ...
              onPress={() => {
                dispatch({
                  type: 'SHOW_PHONE_LOGIN',
                  payload: {componentIdBase: componentId},
                });
              }}
            ...
            <View
              ...
              <View style={{flex: 1}}>
                <BorderlessButton
                  ...
                  <Text style={{marginLeft: 4}}>{lang.toUpperCase()}</Text>
                </BorderlessButton>
              </View>

              <Text
                translationCode="USER_TERMS"
                color="placeholder"
                onPress={() =>
                  dispatch({
                    type: 'SHOW_WEBVIEW',
                    payload: {
                      title: words.USER_TERMS,
                      componentIdBase: componentId,
