  ...
  const obs = of(null).pipe(
    ...
    catchError(err => {
      ...
      if (err && err.code === 'auth/too-many-requests')
        {Alert.alert(words.BLOCK_DEVICE_MULTIPLTE_REQUESTS);}
      return of(err);
    }),
  );
      ...
      <View
        style={{
          ...
          backgroundColor: themeVariables.colors.background,
            ...
            {
                !confirm ?
                    <Animated.View style={{ opacity: isLoading ? 0.7 : 1 }}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
              <RectButton
                ...
                <Text color="primary" fontStyle="bodyEmphasized">
                  {' '}
                  +{countryCode}{' '}
                </Text>
              </RectButton>
            ...
            <Button
              disabled={!isValidPNumber || isLoading}
              loading={isLoading}
          ...
          <Animated.View
            style={{opacity: isLoading2 ? 0.7 : 1, alignItems: 'center'}}>
            ...
            <Button
              disabled={codeConfirmation.length !== 6 || isLoading2}
              loading={isLoading2}
              color={themeVariables.colors.accent}
              onPress={() => {
                //s$.next(phoneNumber);
