...
export default function Profile(props: Props) {
  ...
  const {coverURL, photoURL, bio, displayName, username} = useSelector(
    profileSelector,
  );
  const _onPress = () => {};
  return (
    <ScrollView
      ...
      <View>
        ...
        <Text style={{color: themeVariables.PRIMARY_COLOR}}>
          {' '}
          {displayName}{' '}
        </Text>
      ...
      <Text style={{color: themeVariables.TEXT_COLOR__BIO}}>{bio}</Text>

      <View style={{flexDirection: 'row', paddingHorizontal: 16}}>
        <View style={{flex: 1}}>
          <Button onPress={_onPress}>{words.PROFILE_EDIT_PROFILE}</Button>
        </View>
      </View>
    </ScrollView>
  );
