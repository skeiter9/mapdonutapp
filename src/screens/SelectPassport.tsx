...
const initialItemsSelector = createCachedSelector(
  ...
  (lang, typeList) => {
    const COUNTRIES_BY_CALLING_CODE = (lang: string) =>
      getCountriesInOrderAlphabetic(getCountriesByCallingCode(lang));
    const COUNTRIES_BY_PASSPORT = (lang: string) =>
      getCountriesInOrderAlphabetic(getAllCountries(lang));
    const items = {
...
function ViewList<T>(props: Props) {
  ...
  const scrollTo = (letter: string) => {
    ...
    console.log('scroll to', letter, index, {
  ...
  const content = (
    <>
      ...
      <ScrollView
        ...
        {items.map((c: Section) => (
          <Letter key={c.title} letter={c.title} scrollTo={scrollTo} />
        ))}
      </ScrollView>
    </>;
  );
...
const Letter = ({letter, scrollTo}: LetterProps) => {
  return (
    <ButtonView
      ...
      <Text fontStyle={'caption2'} color="primary">
        {' '}
        {letter}{' '}
      </Text>
    </ButtonView>
