...
import { THEMES_LOGIN_OVERLAY, TUTORIAL_LOGIN, PHONE_LOGIN, PHONE_LOGIN_CODE } from '../screenNames';
import { showOverlay, showModal, pushScreen } from '../utils/rrn';
import config from '../config';
import { OptionsModalPresentationStyle, Navigation, OptionsModalTransitionStyle, LayoutComponent } from 'react-native-navigation';
import { LoginProps } from 'src/screens/Login';

export const showThemesInLogin = (action$: action$) => action$.pipe(
    ofType('OPEN_THEMES_LOGIN'),
    exhaustMap(({ payload: { componentId } }) => {
        return from(showModal(THEMES_LOGIN_OVERLAY, {
            //popGesture: true,
            //blurOnUnmount: true,
...
export const showTutorial = (action$: action$) => action$.pipe(
    ofType('SHOW_TUTORIAL'),
    exhaustMap(({ payload: {componentIdBase} }) => {
        return from(showModal(TUTORIAL_LOGIN, {
            popGesture: true,
            blurOnUnmount: true,
            modalPresentationStyle: OptionsModalPresentationStyle.overCurrentContext,
            modalTransitionStyle: OptionsModalTransitionStyle.crossDissolve,
...
export const showPhoneLogin = (action$: action$, state$: state$) => action$.pipe(
...
export const showPhoneLoginCode = (action$: action$, state$: state$) => action$.pipe(
