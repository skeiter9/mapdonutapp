...
export default (action$: action$, state$: state$) => action$
    .ofType('SHOW_INITIAL_SPLASH')
    .pipe(
...
function getInitialPermissions() {
    if (Platform.OS === 'ios') {
        ...
        ]).then(([notif, cameraStatus, locationWhenInUseStatus,
            mediaLibrary, photoLibrary, siri, calendars, contacts, reminders,
            //mic
        ]) => {
            return ({
                permissions: {
...
function runAfterSplash() {
    const APP_PREFERENCES = ['themeName', 'language', 'ccodeSelected', 'ccodeSelectedCca2', 'phoneNumber', 'bgLogin']
    return Promise.all(
        APP_PREFERENCES.map(name => DefaultPreference.get(name))
    )
        .then(([themeName, language, _ccodeSelected, _ccodeSelectedCca2, _phoneNumber]) => {
            const dispatch = getDispatch();
            const storeState = getStateApp();

            const lang = (language || storeState.languages.lang || LANGUAGE_DEFAULT) as Mapdonut_Languages_Availables;
            const theme = (themeName || storeState.theme.theme || THEME_DEFAULT) as APP_Themes_Availables;

            const deviceDefaultCcode = ((cca2Device) => {
                const ALL_COUNTRIES = getCountriesByCallingCode(storeState.languages.lang);
            ...
            const ccodeSelected = _ccodeSelected || deviceDefaultCcode.ccodeSelected || '1';
            const ccodeSelectedCca2 = _ccodeSelectedCca2 || deviceDefaultCcode.ccodeSelectedCca2 || 'US';
