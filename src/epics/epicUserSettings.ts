...
import { exhaustMap, mapTo, map, filter, tap, switchMap, catchError } from 'rxjs/operators';
import { saveExtraDataUser, getCurrentUserUid } from '../models/user';
import { from, of } from 'rxjs';
import { Navigation, Options, OptionsModalPresentationStyle, OptionsModalTransitionStyle } from 'react-native-navigation';
...
import { Mapdonut_Country, mapdonut_country_available_translations } from '../mapdonut_libs/countries/types';
...
export const updateCurrentUser = (action$: action$, state$: state$) => action$.pipe(
    ofType('UPDATE_CURRENTUSER'),
    exhaustMap(({ payload }) => from(saveExtraDataUser(getCurrentUserUid(), payload)), a => a),
    map(({payload}) => ({ type: 'UPDATE_CURRENTUSER_SUCCESS', payload }))
)

export const changeThemeUser = (action$: action$, state$: state$) => action$.pipe(
    ofType('CHANGE_THEME'),
    tap(({ payload: { theme } }) => {
    ...
    exhaustMap(({ payload, isLogged }) => {
        ...
        if (!isLogged) return of(actionSuccess)
        return from(
            Promise.resolve(actionSuccess)
            .then(() => saveExtraDataUser(getCurrentUserUid(), { themeName: payload.theme }))
...
export const changeLangUser = (action$: action$, state$: state$) => action$.pipe(
    ofType('CHANGE_LANGUAGE'),
    //filter(({ payload }) => payload.isLogged),
    isLoggedPipe(state$),
    exhaustMap(({ payload, isLogged }) => !isLogged ? of(null) : from(saveExtraDataUser(getCurrentUserUid(), { languageSelected: payload.lang })), a => a),
    map(({ payload }) => ({ type: 'UPDATE_CURRENTUSER_SUCCESS', payload }))
)

export const changeUnitUser = (action$: action$, state$: state$) => action$.pipe(
    ofType('CHANGE_UNIT'),
    //filter(({ payload }) => payload.isLogged),
    switchMap(({ payload }) => from(saveExtraDataUser(getCurrentUserUid(),
        payload
    )), a => a),
    map(({ payload }) => ({ type: 'UPDATE_CURRENTUSER_SUCCESS', payload }))
)

export const modifyPassports = (action$: action$, state$: state$) => action$.ofType('MODIFY_PASSPORTS').pipe(
...
export const openModalCountries = (action$: action$, state$: state$) => action$.pipe(
    ...
    exhaustMap(({ payload: { typeList } }) => {
        ...
        */
        ...
        const fuseOptions = (text: string) => ({
            ...
            keys: [
                ...
                mapdonut_country_available_translations.map(lang => ({
                    name: 'raw.translations_dictionary.' + lang, weight: text.length > 3 ? 0.1 : 0.5
                }))
            ),
            maxPatternLength: 12,
        ...
        const selectItem = (item: Mapdonut_Country) => {
            ...
            getDispatch()({ type: 'UPDATE_TMP_DATA', payload: {
        ...
        const nav = () => Navigation.showModal({
            stack: {
                children: [
                    {
                        component: {
